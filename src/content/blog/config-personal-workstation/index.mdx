...
```bash
#ssh连接服务器
sudo ipmitool raw 0x0e 0x65 0 50
```

```cmd
#其他电脑远程控制BMC
ipmitool.exe -I lanplus -H <BMC_IP> -U admin -P <BMC_Password> raw 0x0e 0x65 0 50
```

0x65 后面的0代表手动控速，1为自动(全速)

50代表50%，1代表1%，90代表90%，0x64代表全速

开机时自动设置风扇转速，通过编辑系统的启动脚本实现：

```bash
sudo vim /etc/init.d/fan_control.sh
```
...
sudo ipmitool raw 0x0e 0x65 0 60
...
更新启动脚本列表：使用 `update-rc.d` 命令将脚本添加到系统的启动脚本列表中。
...
https://www.chiphell.com/thread-2604921-1-1.html
...
```bash
CPU_FAN          | 552 RPM           | ok
SYS_FAN_1        | 1242 RPM          | ok
SYS_FAN_2        | 552 RPM           | ok
SYS_FAN_3        | 690 RPM           | ok
...
```bash
#ssh连接服务器
sudo ipmitool raw 0x2e 0x44 0xfd 0x19 0x00 0x02 0x01 0x64
```

```cmd
#其他电脑远程控制BMC
ipmitool.exe -I lanplus -H <BMC_IP> -U root -P <BMC_Password> raw 0x2e 0x44 0xfd 0x19 0x00 0x02 0x01 0x64
```

0x02 is the PWM id from the list attached below in hex from 0x0 - 0x5（对应我上面的结果，CPU_FAN是0x0，SYS_FAN_1是0x02）
...
如果上述命令报错，可能是BMC版本不一样导致的，可以尝试以下命令：(我的主板上以下命令不起作用)

Get/Set PWM Duty

```bash
#ssh连接服务器
sudo ipmitool raw 0x2e 0x5 0xfd 0x19 0x00 <PWM ID> ＜0x00-0x64|0xfe|0xff>

#其他电脑远程控制BMC
ipmitool.exe -I lanplus -H <BMC_IP> -U root -P <BMC_Password> raw 0x2e 0x5 0xfd 0x19 0x00 <PWM ID> ＜0x00-0x64|0xfe|0xff>
```
...
```bash
#ssh连接服务器
sudo ipmitool raw 0x2e 0x05 0xfd 0x19 0x00 0x03 0xfe

#其他电脑远程控制BMC
ipmitool.exe -I lanplus -H <BMC_IP> -U root -P <BMC_Password> raw 0x2e 0x05 0xfd 0x19 0x00 0x03 0xfe
```
...
```bash
#ssh连接服务器
sudo ipmitool raw 0x2e 0x05 0xfd 0x19 0x00 0x00 0x3C

#其他电脑远程控制BMC
ipmitool.exe -I lanplus -H <BMC_IP> -U root -P <BMC_Password> raw 0x2e 0x05 0xfd 0x19 0x00 0x00 0x3C
```



[AutoFan：适配泰安S8030显卡机自动调节系统风扇组件](https://github.com/sonmihpc/AutoFan)
...
```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> user list
```

2.设置bmc用户的用户名

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> user set name 1 admin
```
...
```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> user set password 1 admin
```
...
```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> user disable 1
```
...
```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> user priv user_id> privil
```

bmc执行冷重启

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> mc reset cold
```

bmc热重启

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> mc reset warm
```

6.使用ipmitool设置服务器电源策略

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis policy list
```

查看当前支持的断电机箱电源策略（always-on，always-off，previous）

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis policy always-off
```

将电源策略设置为电源恢复时保持 off状态

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis policy previous
```
...
```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis bootdev
```

查看当前服务器支持的启动项

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis bootdev pxe
```
...
```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis power
```
...
```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis power chassis poh
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis power chassis restart_cause
```
...
```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis power chassis identify
```

11.使用ipmitool工具查看设置bmc IP

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis power lan print 1
```

（查看1通道ip）

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis power lan set 1 ipsrc static
```

(设置1通道ip为静态)

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis power lan set 1 ipaddr 192.168.xx.xx
```

(设置1通道ip)

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis power lan set 1 netmask 255.255.255.0
```

(设置1通道子网掩码)

```cmd
ipmitool.exe -H 192.168.xx.xx -I lanplus -U root -P <Your_IPMI_Password> chassis power lan set 1 defgw ipaddr 192.168.xx.yy
```
...
- `build-essential`包提供了编译程序所需的基本工具，如`gcc`、`g++`等。
- `cmake`是一个跨平台的编译工具，用于生成编译配置文件。
...
1. 去官网查找cuda安装包：[CUDA Toolkit Downloads | NVIDIA Developer](https://developer.nvidia.com/cuda-downloads)，根据`nvidia-smi`的cuda版本进行选择。
   ...
   如果安装过程中报错`Failed to verify gcc version. See log at /var/log/cuda-installer.log for details.`：

   执行`sudo apt install build-essential`即可解决。
...
使用 `sudo docker info`查看docker系统信息，如果从结果中看到了你配置的加速地址，说明配置成功。
...
Docker默认不支持GPU，需先安装NVIDIA官方提供的容器工具包。需要确保已经安装了Nvidia Driver。
...
```bash
sudo docker run -it --name test_nvidia_docker --gpus all nvidia/cuda:12.4.1-cudnn-devel-ubuntu22.04
```

其中最后的参数`nvidia/cuda:12.4.1-cudnn-devel-ubuntu22.04` 是Nvidia官方的镜像，需要根据工作站主机中实际安装的CUDA版本进行修改，版本可以用`nvcc -V`查看。

进入容器之后运行 `nvidia-smi` 命令，有输出即为成功。
...
Chrome浏览器新建管理员时出现`Unable to create administrator user`错误 / 登录时出现`Unable to login`错误：

尝试清除该网页的浏览器缓存：F12打开开发者工具--切换到"应用"栏--点击"存储"中的"清除网站数据"，刷新页面即可解决该问题。

### 3.6 安装Docker Desktop
